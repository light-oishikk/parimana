...
Key architectures relevant to quantum imaging include:

\begin{itemize}
    \item \textbf{Convolutional Neural Networks (CNNs):} Learn hierarchical spatial features through convolutional filters, enabling effective image-to-image mappings for denoising and reconstruction \citep{lecun2015}.
    \item \textbf{U-Net:} An encoder--decoder architecture with skip connections that preserves fine-grained spatial information, widely adopted for biomedical and computational imaging \citep{ronneberger2015}.
    \item \textbf{Generative Adversarial Networks (GANs):} A generator--discriminator framework that produces perceptually realistic reconstructions by learning the distribution of natural images \citep{goodfellow2014}.
    \item \textbf{Autoencoders:} Learn compressed latent representations from which images can be reconstructed, relevant to dimensionality reduction in quantum measurement data.
\end{itemize}
